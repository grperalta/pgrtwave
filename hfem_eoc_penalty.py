...
def hybrid_error_table():
    ...
    list_pen = [1e-6, 1e-7, 1e-8, 1e-9, 1e-10, 1e-11]

    error = np.zeros((len(list_pen), 6)).astype(float)
    order = np.zeros((len(list_pen)-1, 5)).astype(float)

    i = 0
    for pen in list_pen:
        ocp = OCP(prm=dict(pen=pen, n=65, T=1.0, num_time_step=1000, hybrid=True,
